...
public class Morphology_ extends PlugInFrame implements ActionListener, ChangeListener {
                ...
                for (int x = -raggio; x <= raggio; x++)
                    ...
                    // if( se.get(x+raggio,y+raggio)!=0 )
                    {
                        if ((s - x) >= r.y && (s - x) < (r.y + r.height) && (t - y) >= r.x && (t - y) < (r.x + r.width)) {
                            if (se.get(x + raggio, y + raggio) != 0 && max < ((img[s - x][t - y] & 0xff) + se.get(x + raggio, y + raggio)))
                                max = (img[s - x][t - y] & 0xff) + se.get(x + raggio, y + raggio);
                        }
                        if (max > 255) max = 255;
                    }
            ...
            for (int j = r.x; j < (r.x + r.width); j++) {
                ...
                for (int x = -raggio; x <= raggio; x++)
                    for (int y = -raggio; y <= raggio; y++)
                        if (se.get(x + raggio, y + raggio) != 0) {
                            if ((s + x) >= r.y && (s + x) < (r.y + r.height) && (t + y) >= r.x && (t + y) < (r.x + r.width)) {
                                if (min > ((img[s + x][t + y] & 0xff) + se.get(x + raggio, y + raggio)))
                                    min = (img[s + x][t + y] & 0xff) - se.get(x + raggio, y + raggio);
                            }
            ...
            for (int j = r.x; j < (r.x + r.width); j++) {
                ...
                for (int x = -raggio; x <= raggio; x++)
                    for (int y = -raggio; y <= raggio; y++) {
                        ...
                        if ((s - x) >= r.y && (s - x) < (r.y + r.height) && (t - y) >= r.x && (t - y) < (r.x + r.width))
                            if ((img[s - x][t - y] & 0xff) > 0 && se.get(x + raggio, y + raggio) != 0)
                                intersection = true; //mg[s-x][t-y]&0xff)>0 è foreground ; se.get(x+raggio,y+raggio)!=0 è foreground
                    }
                if (intersection)
        ...
        for (int i = r.y; i < (r.y + r.height); i++) {
            for (int j = r.x; j < (r.x + r.width); j++) {
                ...
                for (int x = -raggio; x <= raggio; x++)
                    for (int y = -raggio; y <= raggio; y++) {
                        if ((s + x) >= r.y && (s + x) < (r.y + r.height) && (t + y) >= r.x && (t + y) < (r.x + r.width)) {
                            if (se.get(x + raggio, y + raggio) != 0 && (img[s + x][t + y] & 0xff) < 250)
                                contained = false; //metto <250 e non !=255 perche alcune immagini sebbene a 2 livelli possono avere ad es tutti 254 aql posto di 255...;se.get(x+raggio,y+raggio)!=0 è foreground; (img[s+x][t+y]&0xff)<250 è background
    ...
    public void boundaryExtraction(byte[] pixels, ImageProcessor ip, StructuringElement se) {
        ...
        for (int i = 0; i < imgHeight; i++) {
            for (int j = 0; j < imgWidth; j++) {
                if (((pixelsIniziali[i * imgWidth + j] & 0xff) - (pixels[i * imgWidth + j] & 0xff)) < 0)
                    pixels[i * imgWidth + j] = 0;
                else
                    pixels[i * imgWidth + j] = (byte) ((pixelsIniziali[i * imgWidth + j] & 0xff) - (pixels[i * imgWidth + j] & 0xff));
            }
        }
    }
    ...
    public void gradient(byte[] pixels, ImageProcessor ip, StructuringElement se) {
        ...
        for (int i = 0; i < imgHeight; i++) {
            for (int j = 0; j < imgWidth; j++) {
                if (((pixels[i * imgWidth + j] & 0xff) - (pixels2[i * imgWidth + j] & 0xff)) < 0)
                    pixels[i * imgWidth + j] = 0;
                else
                    pixels[i * imgWidth + j] = (byte) ((pixels[i * imgWidth + j] & 0xff) - (pixels2[i * imgWidth + j] & 0xff));
            }
        }
    }
    ...
    public void setMask(int[][] m, int grayValue) {
        ...
        for (int i = 0; i < mask.length; i++)
            for (int j = 0; j < mask.length; j++)
                if (m[i][j] == Morphology_.SET)
                    mask[i][j] = grayValue;//elem. strutturale standard o disegnato dall'utente
